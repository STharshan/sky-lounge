    ...
    <>
      ...
      {visible && (
        <div className="fixed bottom-4 left-4 right-4 md:bottom-6 text-center md:right-6 md:left-auto max-w-full md:max-w-xs p-4 rounded-lg 
                         shadow-lg z-50 
                      bg-[#1B3A2D] transition-colors border border-[#B8975A]/20">
          <p className="text-sm mb-2 text-center text-[#F0E0B0]">
            We use cookies to improve your experience.{" "}
          </p>
          <p className="mb-3">
            <Link
              to="/privacy"
              className="underline text-[#B8975A] hover:text-[#F0E0B0] transition-colors"
            >
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3">
            <button
              ...
              className="bg-transparent border border-[#B8975A] text-[#F0E0B0] px-4 py-2 rounded text-sm hover:bg-[#B8975A]/10 transition"
            >
              Reject
            </button>
            <button
              onClick={handleAccept}
              className="bg-[#B8975A] text-[#0A1F12] px-4 py-2 rounded text-sm 
                         hover:bg-[#F0E0B0] transition"
            >
              Accept
            </button>
          </div>
      ...
      {showIcon && !visible && (
        <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-40">
          <button
            ...
            className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-[#B8975A] shadow-lg border border-[#0A1F12] 
          ...
          >
            <img
              ...
              alt="Cookie Icon"
              className="w-5 h-5 sm:w-6 sm:h-6 object-contain brightness-0 filter" // filter to make svg black/dark to contrast with gold
            />
          </button>
        </div>
      )}
